...
namespace app
{
  ...
  //      If I'm going to draw only the box I'm editing and the skybox, I guess I want to keep the old CmdLine around
  //      on the CPU to load when I'm done.  How do I transfer my edits from the temporary CmdLine to the old one when
  //      I'm done?  If I keep the old CmdLine around, I can always offer the chance to discard all changes.  I guess I
  //      could keep an iterator to the aabb I'm editing (and probably its material too) when I copy it to the temporary
  //      CmdLine.
  //
  //      I'm going to select an aabb by path tracing on the CPU.  If that eventually gets too expensive, I could imagine
  //      returning an index to the closest aabb from an OpenCL kernel instead.
  //
  //      For now, materials just have 2 colors to edit.  ImGui has built-in color editors I can use.  Eventually, I'll
  //      want to select textures with a file selector.
  //
  //      If I edit a material that's used for multiple boxes, all of those boxes will change.  It seems like that's the
  //      default behavior I want, but I'm worried that it might confuse new users.  I do want an option to copy a material
  //      into a new material and imeediately start using that material for the box I'm editing.  Sounds like Save As.
  //
  //      Being able to move a box around with the mouse sounds like a much better second step than being able to resize it
  //      with the mouse.  I think I can figure out which way the mouse is moving by relating it to the focal plane.  The
  //      cursor moves in the focal plane, and the focal plane is defined in global coordinates.
  //
  //      Realized that I need to show other volumes for moving a box around the scene.  So, I'll just be editing the CmdLine
  //      for now rather than making a copy.  I can try making a copy and rendering onto the last framebuffer before editing
  //      mode if I later realize that framerate is limited by uploads to the GPU.  So, I just have to remember to upload data
  //      to the GPU whenever I do something in response to mouse motion in editing mode.  Reacting correctly to changes based
  //      on the GUI will require more careful bookkeeping.
  //
  //      I need 2 distinct ways to interpret mouse motion now: CameraController and edit mode.  This problem seems too simple
  //      to benefit from a state machine design.  In edit mode, I could just not call handleCamera().  In fact, I'm tempted to
  //      write a handleEdit() function instead with a GUI button to enter edit mode.  I can think more about the most convenient
  //      semantics for edit mode once I've got more of the details working.
  //
  //      I'm tempted to write an edit mode window that's started from the main menu bar.  As long as the edit mode window is
  //      open, mouse clicks and motion select and drag boxes instead of controlling the camera.  Later, I want a convenient way
  //      to control the camera and maybe a spherical CameraController centered on the object I'm editing.  I guess I'm really
  //      describing a model edit mode and a global edit mode.  Global edit mode just lets me move boxes or groups of boxes and
  //      wants to render everything in the scene.  Model edit mode just renders a smaller group of boxes and is focused on
  //      trying out new material properties, box sizes, or maybe even moving elements of the model.  This almost sounds like
  //      a context menu with a switch statment for which mouse mode I'm in.  I'm envisioning other changes on entering one of
  //      these 3 application modes, so this is starting to sound more like a state machine.
  //
  //      Maybe a "selection" would be a better way to organize what I want to do?  I do eventually want the ability to select
  //      multiple boxes to work on at once.  Something like CTRL+click could try to trigger a selection.  On selection, a GUI
  //      window will pop up with a "focus" button for model edit mode.
}
